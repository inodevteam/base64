...
const bool base64_init (base64_ctx*ctx, const uint8_t method) {
  ...
  if (method!=BASE64_ENCODE&&method!=BASE64_DECODE) {
    printf("%s: %s\n",__FUNCTION__,msg[1]);
    return false;
  }
  ctx->method=method;
    ...
    case BASE64_DECODE:
      if (ctx->method==BASE64_DECODE) {
        if (base64_convert(c,ctx->method)<0||c==NULL_CHAR) {
          ctx->buf[ctx->buf_len]=END_OF_STRING;
          ctx->buf_len++;
          break;
        }
      }
      switch (ctx->mode) {
        case 0:
          if (ctx->method==BASE64_ENCODE) {
            ctx->buf[ctx->buf_len]=base64_convert((c>>2)&0x3f,ctx->method);
            ctx->buf_len++;
          }
          break;
        case 1:
          ctx->buf[ctx->buf_len]=(ctx->method==BASE64_ENCODE?base64_convert(((ctx->tmp<<4)|(c>>4)&0xf)&0x3f,ctx->method):(base64_convert(ctx->tmp,ctx->method)<<2)|(base64_convert(c,ctx->method)>>4));
          ctx->buf_len++;
          break;
        case 2:
          if (ctx->method==BASE64_ENCODE) next=false;
          ctx->buf[ctx->buf_len]=(ctx->method==BASE64_ENCODE?base64_convert(((ctx->tmp<<2)|(c>>6)&0x3)&0x3f,ctx->method):(base64_convert(ctx->tmp,ctx->method)<<4)|(base64_convert(c,ctx->method)>>2));
          ctx->buf_len++;
          break;
        case 3:
          ctx->buf[ctx->buf_len]=(ctx->method==BASE64_ENCODE?base64_convert(c&0x3f,ctx->method):(base64_convert(ctx->tmp,ctx->method)<<6)|(base64_convert(c,ctx->method)));
          ctx->buf_len++;
          break;
      }
      break;
